...
@Path("/{ListarMisOKOs : (?i)listarmisokos}")
public class ListarMisOKOs {

    @POST
    @Produces(MediaType.APPLICATION_JSON)
    public RespuestaWS<List<RespuestaWSDispositivo>> listarMisOKOs(@FormParam("apikey") String apiKey) {

        RespuestaWS<List<RespuestaWSDispositivo>> response = new RespuestaWS<>();
        try {
            response.responseStatus = 0;
            response.message = GestionDispositivos.ListarDispositivosUsuario(apiKey);
            response.error = "";
            return response;
